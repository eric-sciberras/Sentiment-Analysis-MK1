...
def neuralnet(filename):
    ...
    X_train = sequence.pad_sequences(
        X[:int(training_size*len(X))], maxlen=max_words)
    X_test = sequence.pad_sequences(
        X[-int((1-training_size)*len(X)):], maxlen=max_words)

    Y_train = sequence.pad_sequences(Y[:int(training_size*len(Y))], maxlen=1)
    Y_test = sequence.pad_sequences(
        Y[-int((1-training_size)*len(Y)):], maxlen=1)

    embedding_size = 32
    model = Sequential()
    model.add(Embedding(len(tokenizer.word_index.items()) +
                        1, embedding_size, input_length=max_words))
    model.add(LSTM(100))
    ...
    checkpoint = ModelCheckpoint(
        filepath, monitor='val_acc', verbose=1, save_best_only=False, mode='max')
